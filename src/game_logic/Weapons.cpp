...
void DefaultWeapon::TrySpawnBullet(Scene& scene, EntityRegistry& registry) {
  if (time_from_last_shot_ >= cooldown_) {
    ...
    bullet.AddComponent<RenderComponent>(
        std::bind(&DefaultBulletDrawer::Render, DefaultBulletDrawer{bullet}, std::placeholders::_1));
...
void DefaultWeapon::Cooldown(TimeEvent event) { time_from_last_shot_ += event.dt; }